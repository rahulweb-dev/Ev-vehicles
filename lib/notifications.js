...
async function smtpBroadcast({ subject, html }) {
  ...
  await broadcastMail({ emails, subject, html });

  // Trigger the mail worker immediately so emails go out now, not at 2 AM cron time.
  // Fire-and-forget — the worker handles rate limiting and retries internally.
  const secret = process.env.SCHEDULER_SECRET || process.env.CRON_SECRET;
  if (secret) {
    fetch(`${SITE_URL}/api/admin/mail-worker?secret=${secret}`, { method: "GET" })
      .catch(err => console.error("[notify] mail-worker trigger failed:", err.message));
  }
}
