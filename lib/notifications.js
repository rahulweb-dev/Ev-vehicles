...
const BATCH    = 50; // Resend free tier: 50/batch

async function getActiveSubscribers() {
  const dbConnect  = (await import("@/lib/mongodb")).default;
  const Subscriber = (await import("@/lib/models/Subscriber")).default;
  await dbConnect();
  return Subscriber.find({ status: "active" }).select("email").lean();
}

async function resendBroadcast({ subject, html }) {
  ...
  for (let i = 0; i < subscribers.length; i += BATCH) {
    ...
    await Promise.allSettled(
      batch.map(s => resend.emails.send({ from, to: s.email, subject, html }))
    );
  }
}
