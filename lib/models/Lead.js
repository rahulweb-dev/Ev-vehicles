...
// Dealer-scoped queries filter by city and/or state — without these indexes
// every dealer GET does a full collection scan.
LeadSchema.index({ city: 1, createdAt: -1 });
LeadSchema.index({ state: 1, createdAt: -1 });
// Admin analytics: countDocuments({ createdAt: { $gte: ... } }) — standalone createdAt range filter
