...
import {
  ...
  Building2, FileText, Shield, Bike,
} from "lucide-react";
...
const INSURANCE_BASE = 0.02;   // 2% for two-wheelers (lower than cars)
const EV_INS_DISC    = 0.12;

export default function BikeCityPriceClient({ vehicle, cityData, allCities }) {
  ...
  const prices = useMemo(() => {
    ...
    const smart  = 500;
    const charger = 2500;  // portable charger
    const misc   = 1500;
    const total  = ex + reg + ins + smart + charger + misc;

    return { ex, reg, ins, smart, charger, misc, total };
  }, [variant, cityData]);

  const rows = prices ? [
    { label: "Ex-Showroom Price",                                          value: fmt(prices.ex),      icon: Bike,          note: "Base price before taxes" },
    ...
    { label: "Portable Charger",                                           value: fmt(prices.charger), icon: BatteryCharging, note: "Home charging cable" },
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="mx-auto max-w-4xl">
        ...
        <nav className="mb-6 flex items-center gap-1 text-xs text-gray-400">
          ...
          <Link href="/bikes" className="hover:text-gray-600">Electric Bikes & Scooters</Link>
          <ChevronRight size={12} />
          <Link href={`/bikes/${vehicle.slug}`} className="hover:text-gray-600">{vehicle.name}</Link>
          <ChevronRight size={12} />
          ...
          {prices ? (
            <div className="p-4">
              ...
              <p className="mt-3 flex items-start gap-1 text-[10px] text-gray-400">
                ...
                Prices are indicative. Actual charges may vary by dealer. Check with your nearest authorised dealer for exact on-road pricing.
              </p>
            </div>
          ) : (
          ...
          <div className="rounded-3xl bg-white border border-gray-200 shadow-sm p-5 mb-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                { label: "Battery",     value: vehicle.performance?.batteryCapacity },
                { label: "Motor Power", value: vehicle.performance?.power },
        ...
        <div className="rounded-3xl bg-white border border-gray-200 shadow-sm p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {allCities
              ...
              .map(c => (
                <Link key={c.slug}
                  href={`/bikes/${vehicle.slug}/price-in-${c.slug}`}
