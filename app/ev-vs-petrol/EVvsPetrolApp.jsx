...
export default function EVvsPetrolApp() {
  ...
  const monthly = useMemo(() => {
    ...
    const co2Saved   = (km * 12) * (2.31 / fuelEff) / 1000
      ...
      <div className="mx-auto max-w-5xl px-4 py-10">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            ...
            {[
              { label: 'EV Price', icon: <Zap size={13} className="text-green-500" />, value: evPrice, set: setEvPrice, min: 50000, max: 8000000, step: 10000, display: `₹${fmt(evPrice)}`, color: 'accent-green-600' },
              { label: 'Comparable Petrol Car Price', icon: <Fuel size={13} className="text-orange-500" />, value: petrolPrice, set: setPetrolPrice, min: 50000, max: 8000000, step: 10000, display: `₹${fmt(petrolPrice)}`, color: 'accent-orange-500' },
            ].map(f => (
              <div key={f.label} className="mb-5">
                <div className="flex items-center justify-between mb-1">
                  <label className="text-sm font-semibold text-gray-700 flex items-center gap-1.5">{f.icon} {f.label}</label>
                  <span className="text-sm font-bold text-gray-800">{f.display}</span>
                </div>
                <input type="range" min={f.min} max={f.max} step={f.step} value={f.value} onChange={e => f.set(Number(e.target.value))} className={`w-full ${f.color}`} />
              </div>
            ))}
            ...
            <div className="mb-5">
              ...
              <input type="range" min="5" max="200" step="5" value={dailyKm} onChange={e => setDailyKm(Number(e.target.value))} className="w-full accent-green-600" />
            </div>
            ...
            <div className="mb-5">
              ...
              <input type="range" min="80" max="150" step="1" value={fuelPrice} onChange={e => setFuelPrice(Number(e.target.value))} className="w-full accent-orange-500" />
            </div>
            ...
            <div className="mb-5">
              ...
              <input type="range" min="8" max="30" step="0.5" value={fuelEff} onChange={e => setFuelEff(Number(e.target.value))} className="w-full accent-orange-500" />
            </div>
            ...
            <div className="mb-5">
              ...
              <input type="range" min="4" max="15" step="0.5" value={elecPrice} onChange={e => setElecPrice(Number(e.target.value))} className="w-full accent-green-600" />
            </div>
            ...
            <div>
              ...
              <input type="range" min="3" max="12" step="0.5" value={evEff} onChange={e => setEvEff(Number(e.target.value))} className="w-full accent-green-600" />
          ...
          <div className="flex flex-col gap-4">
            <div className={`rounded-2xl p-6 text-center ${monthly.savings > 0 ? 'border border-green-200 bg-green-50' : 'border border-orange-200 bg-orange-50'}`}>
              ...
              <p className="text-xs text-gray-500 mt-2">{fmt(monthly.km)} km/month · EV ₹{fmt(monthly.evCost)} vs Petrol ₹{fmt(monthly.petrolCost)}</p>
            </div>
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <div className="h-2.5 rounded-full bg-gray-100 overflow-hidden"><div className="h-full bg-orange-400 rounded-full w-full" /></div>
                </div>
              </div>
              <p className="text-xs text-gray-400 mt-3">EV ₹{fmt(monthly.evCost / monthly.km, 1)}/km · Petrol ₹{fmt(monthly.petrolCost / monthly.km, 1)}/km</p>
            </div>
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
              ...
              <div className="space-y-3">
                {[
                  { label: 'Price Difference (EV premium)', value: monthly.priceDiff >= 0 ? `+₹${fmt(monthly.priceDiff)}` : `-₹${fmt(Math.abs(monthly.priceDiff))}`, icon: IndianRupee, color: monthly.priceDiff >= 0 ? 'text-red-600 bg-red-50' : 'text-green-600 bg-green-50' },
                  { label: 'Break-Even Point', value: monthly.savings <= 0 ? 'Never' : monthly.priceDiff <= 0 ? 'Instant' : `${fmt(monthly.breakEven, 1)} months`, icon: Clock, color: 'text-blue-600 bg-blue-50' },
                  { label: '5-Year Net Savings', value: yr5Savings > 0 ? `₹${fmt(yr5Savings)}` : `-₹${fmt(Math.abs(yr5Savings))}`, icon: TrendingDown, color: yr5Savings > 0 ? 'text-green-600 bg-green-50' : 'text-red-600 bg-red-50' },
                  { label: '10-Year Net Savings', value: yr10Savings > 0 ? `₹${fmt(yr10Savings)}` : `-₹${fmt(Math.abs(yr10Savings))}`, icon: Leaf, color: yr10Savings > 0 ? 'text-green-600 bg-green-50' : 'text-red-600 bg-red-50' },
                ].map(row => (
                  <div key={row.label} className="flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl ${row.color}`}><row.icon size={14} /></div>
        ...
        <section className="mt-10 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
          ...
          <div className="grid sm:grid-cols-2 gap-4 text-sm text-gray-600">
            {[
              { label: 'Monthly Petrol Cost', formula: '(daily km × 30) ÷ mileage × petrol price', color: 'bg-orange-50 text-orange-800' },
              { label: 'Monthly EV Charging Cost', formula: '(daily km × 30) ÷ EV efficiency × electricity rate', color: 'bg-green-50 text-green-800' },
              { label: 'Break-even Point', formula: 'price difference ÷ monthly savings (in months)', color: 'bg-blue-50 text-blue-800' },
              { label: 'Net 5-Year Savings', formula: '(monthly savings × 60) − price difference', color: 'bg-purple-50 text-purple-800' },
            ].map(f => (
              <div key={f.label}>
                <p className="font-bold text-gray-800 mb-1">{f.label}</p>
                <code className={`block rounded-lg px-3 py-2 text-xs font-mono ${f.color}`}>{f.formula}</code>
              </div>
            ))}
          </div>
        ...
        <section className="mt-6 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
          ...
          {[
            { q: "How much cheaper is it to charge an EV vs fill petrol in India?", a: "At current prices (petrol ≈ ₹106/litre, electricity ≈ ₹8/kWh), an EV costs roughly ₹1.3–1.5 per km while a petrol car costs ₹5–8 per km. This means EV running costs are 70–80% lower than petrol for average Indian conditions." },
            { q: "What is the break-even point for an EV in India in 2026?", a: "For most Indian EV buyers driving 40–50 km daily, the break-even point is typically 3–5 years. With state subsidies under PM E-DRIVE and FAME-III (expected), the break-even can reduce to 2–3 years." },
            { q: "Do EVs have lower maintenance costs than petrol cars?", a: "Yes, significantly. EVs have fewer moving parts — no engine oil changes, no spark plugs, no exhaust system, no gearbox servicing. Studies suggest EVs cost 30–50% less to maintain than petrol cars over 5 years." },
            { q: "Is EV charging available everywhere in India?", a: "As of 2026, India has over 12,000 public charging stations. EESL and ChargeZone are expanding rapidly in Tier 2 cities. Most EV owners charge at home overnight using a standard 15A socket or a dedicated home charger." },
          ].map((faq, i) => (
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          <Link href="/emi-calculator" className="flex items-center gap-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm hover:border-green-300 transition">
            ...
            <div><p className="font-bold text-gray-800">EV EMI Calculator</p><p className="text-xs text-gray-500">Calculate your monthly loan payment</p></div>
          ...
          <Link href="/ev-subsidy" className="flex items-center gap-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm hover:border-green-300 transition">
            ...
            <div><p className="font-bold text-gray-800">State EV Subsidies</p><p className="text-xs text-gray-500">Reduce your EV price further</p></div>
            <ChevronRight size={16} className="ml-auto text-gray-300" />
          </Link>
        </div>
      </div>
